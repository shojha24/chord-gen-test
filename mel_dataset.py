...
class ChordMatchedDataset(Dataset):
    def __init__(self, track_path, annotation_path, sample_rate, hop_length, n_mels, n_fft, cache_path="mel_dataset.hdf5"):
        super().__init__()
    ...
    def process_single_file(self, file_num, instrument, track_path, annotation_data, sample_rate, hop_length, 
        ...
        # audio path should be the singular audio file in track path that has file_num in it (the label after can be anything, so mix won't work)

        audio_path = os.path.join(track_path, f"{file_num:04d}_{instrument}.flac")
    ...
    def preprocess_and_view_parallel(self):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
            ...
            file_numbers = df['song_number'].tolist()
            instruments = df['instrument'].tolist()
            results = list(executor.map(process_func, file_numbers, instruments))
...
if __name__ == "__main__":
    ...
    track_path = "dataset\\tracks"

    dataset = ChordMatchedDataset(track_path, annotation_path, config.SAMPLE_RATE, config.HOP_LENGTH, config.N_MELS, config.N_FFT)
