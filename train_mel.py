...
from mel_dataset import ChordMatchedDataset
...
def get_ds(track_path, annotation_path, batch_size=config.BATCH_SIZE, sample_rate=config.SAMPLE_RATE, hop_length=config.HOP_LENGTH, n_mels=config.N_MELS, n_fft=config.N_FFT):
    dataset = ChordMatchedDataset(track_path, annotation_path, sample_rate, hop_length, n_mels, n_fft)
...
def train_model(track_path="dataset\\tracks", annotation_path="dataset\\annotations", experiment_name="runs/music_transformer", num_epochs=config.NUM_EPOCHS, lr=config.LEARNING_RATE, num_classes=config.NUM_CLASSES):
    ...
    dataset, train_dataloader, val_dataloader = get_ds(track_path, annotation_path)
    ...
    for epoch in range(initial_epoch, num_epochs):
        ...
        model_filename = f"music_models/mel_epoch_{epoch + 1}.pt"
        torch.save({
            'epoch': epoch,
